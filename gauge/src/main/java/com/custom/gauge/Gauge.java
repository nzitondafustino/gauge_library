...
public class Gauge extends View {
    ...
    public Gauge(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        TypedArray attributes = context.getTheme().obtainStyledAttributes(attrs, R.styleable.Gauge, defStyleAttr, 0);
    ...
    protected void initByAttributes(TypedArray attributes) {
        finishedStrokeColor = attributes.getColor(R.styleable.Gauge_arc_finished_color, default_finished_color);
        unfinishedStrokeColor = attributes.getColor(R.styleable.Gauge_arc_unfinished_color, default_unfinished_color);
        textColor = attributes.getColor(R.styleable.Gauge_arc_text_color, default_text_color);
        textSize = attributes.getDimension(R.styleable.Gauge_arc_text_size, default_text_size);
        arcAngle = attributes.getFloat(R.styleable.Gauge_arc_angle, default_arc_angle);
        setMax(attributes.getInt(R.styleable.Gauge_arc_max, default_max));
        setProgress(attributes.getFloat(R.styleable.Gauge_arc_progress, 0));
        strokeWidth = attributes.getDimension(R.styleable.Gauge_arc_stroke_width, default_stroke_width);
        suffixTextSize = attributes.getDimension(R.styleable.Gauge_arc_suffix_text_size, default_suffix_text_size);
        suffixText = TextUtils.isEmpty(attributes.getString(R.styleable.Gauge_arc_suffix_text)) ? default_suffix_text : attributes.getString(R.styleable.Gauge_arc_suffix_text);
        suffixTextPadding = attributes.getDimension(R.styleable.Gauge_arc_suffix_text_padding, default_suffix_padding);
        bottomTextSize = attributes.getDimension(R.styleable.Gauge_arc_bottom_text_size, default_bottom_text_size);
        bottomText = attributes.getString(R.styleable.Gauge_arc_bottom_text);
    }
    ...
    public void setProgress(float progress) {
        ...
        if (this.progress > getMax()) {
            ...
            if(Math.abs(this.progress) > getMax()){
                this.progress = -1 * getMax();
            }
        }
        invalidate();
    }
    ...
    protected void onDraw(Canvas canvas) {
        ...
        float finishedSweepAngle = Math.abs(progress)/(float) getMax() * arcAngle;
